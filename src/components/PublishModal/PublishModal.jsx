...
const PublishModal = ({ open, onClose, noteId, noteTitle, getHtmlContent, getJsonContent, isPublished }) => {
    ...
    const handlePublish = useCallback(async () => {
        ...
        try {
            ...
            // Backend returns statuses even on 502 (all-fail) and 207 (partial)
            const data = err.response?.data;
            if (data?.statuses) {
                setStatuses(data.statuses);
                const failedErrors = data.statuses.filter(s => s.status === 'failed').map(s => s.error);
                toast.error(failedErrors[0] || 'All channels failed to publish');
            } else {
    ...
    const handleRetry = useCallback(async (channelType) => {
        ...
        try {
            ...
            if (data?.statuses?.[0]) {
                setStatuses((prev) =>
                    prev.map((s) => (s.type === channelType ? data.statuses[0] : s))
                );
                toast.error(data.statuses[0].error || `${CHANNEL_META[channelType].name} failed again`);
            } else {
